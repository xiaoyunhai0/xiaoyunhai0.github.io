const anzhiyu = {
  ...
  // 获取音乐页播放器实例，兼容 Meting 和本地 JSON 两种初始化方式
  getMusicPageAplayer: function () {
    const anMusicPage = document.getElementById("anMusic-page");
    if (!anMusicPage) return null;

    const meting = anMusicPage.querySelector("meting-js");
    if (meting && meting.aplayer) return meting.aplayer;

    const localPlayer = anMusicPage.querySelector("#anMusic-page-aplayer");
    if (localPlayer && localPlayer.aplayer) return localPlayer.aplayer;
  ...
  // 使用本地 JSON 初始化音乐页播放器
  initCustomMusicPagePlayer: async function () {
    const anMusicPageMeting = document.getElementById("anMusic-page-meting");
    if (!anMusicPageMeting || typeof APlayer !== "function") return;

    const songs = await anzhiyu.getCustomMusicList();
    anMusicPageMeting.innerHTML = '<div id="anMusic-page-aplayer"></div>';
    const playerContainer = document.getElementById("anMusic-page-aplayer");
    playerContainer.aplayer = new APlayer({
  ...
  // 将音乐缓存播放
  cacheAndPlayMusic() {
    anzhiyu.getCustomMusicList().then(songs => anzhiyu.playMusic(songs));
  },
  // 播放音乐
  playMusic(songs) {
    const metingAplayer = anzhiyu.getMusicPageAplayer();
    if (!metingAplayer || !songs.length) return;
  ...
  getCustomPlayList: function () {
    ...
    const anMusicPageMeting = document.getElementById("anMusic-page-meting");
    if (GLOBAL_CONFIG.music_page_default === "custom" && !urlParams.get("id") && !urlParams.get("server")) {
      anzhiyu.initCustomMusicPagePlayer();
  ...
  addEventListenerMusic: function () {
    ...
    // 默认加载的歌单
    if (GLOBAL_CONFIG.music_page_default === "custom" && !anMusicPage.querySelector("#anMusic-page-aplayer")) {
      anzhiyu.changeMusicList();
    }
